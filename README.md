...
- `supabase/migrations/20260904120000_booking_party_contact.sql` — authenticated-only `booking_party_contact` view (couple name + phone for a shared booking). **Already on live** (Syber confirmed). Live version is `20260904172239_booking_party_contact` — dashboard timestamp, not the git filename. Do **not** re-push.
...
Every migration in this repo is already live on Dahr LY. They were applied through the dashboard, so Dahr LY recorded different version numbers and `migration list` shows no version in common with this repo (git filenames vs live `schema_migrations` timestamps). Reconcile the history once with `supabase migration repair` before pushing or before enabling automatic deploys — the exact commands, and the GitHub ↔ Supabase setup they unblock, are in [`docs/supabase-github.md`](docs/supabase-github.md). Do **not** `db push` SQL that is already applied.
